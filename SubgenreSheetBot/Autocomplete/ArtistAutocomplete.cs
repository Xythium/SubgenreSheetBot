...
public class ArtistAutocomplete : AutocompleteHandler
{
    ...
    public ArtistAutocomplete(SheetService sheetService)
    {
    ...
    {
        var artists = SheetService.GetMostCommonArtists();
        var value = autocompleteInteraction.Data.Current.Value.ToString();

        IEnumerable<string> results;
        if (string.IsNullOrWhiteSpace(value))
            results = artists.Take(25);
        else
            results = artists.Where(sg => sg.Contains(value, StringComparison.OrdinalIgnoreCase)).Take(25);
